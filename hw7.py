...
Код должен быть написан так, чтобы легко масштабироваться для обработки
любого количества карточек товаров.
Функция calculate_total_price() принимает на вход документ html
и должна вернуть искомое значение.
...
def calculate_total_price(html: str) -> float:
    ...
    books = soup.find_all('div', 'book-card')

    total_price = 0

    for book in books:
        price = book.find('p', 'count price').text.lstrip('Цена: $')
        count_stock = (
            book
            .find('p', 'count stock')
            .text
            .lstrip('Количество на складе: ')
        )
        price_for_books = float(price) * int(count_stock)
        total_price += price_for_books
